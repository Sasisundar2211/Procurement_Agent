...
def _build_summary(drifts: pd.DataFrame) -> pd.DataFrame:
    ...
    top_indices = list(drifts.index[:max_summaries])

    if top_indices:
        with ThreadPoolExecutor(max_workers=min(5, len(top_indices))) as executor:
            future_to_idx = {
                executor.submit(
                    summarize_drift_with_gemini,
                    drifts.loc[idx, "contract_unit_price"],
                    drifts.loc[idx, "unit_price"],
                ): idx
                for idx in top_indices
            }
            for future in as_completed(future_to_idx, timeout=1.5):
                idx = future_to_idx[future]
                try:
                    summary = future.result(timeout=0.1)
                    drifts.at[idx, "gemini_summary"] = summary
                except Exception as exc:  # noqa: BLE001
                    logger.warning("Failed or timed out fetching summary for index %s: %s", idx, exc)
